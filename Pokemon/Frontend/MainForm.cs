...
namespace Pokemon
{
	partial class MainForm : Form
	{
		...
		private Poke EnemyPoke;

		private Poke[] MyParty = new Poke[6] {new Poke("ギルガルド盾"), new Poke("ゲッコウガ"), new Poke("カメックス"),
			new Poke("ピカチュウ"), new Poke("ジュカイン"), new Poke("タマタマ")};
		private Poke[] EnemyParty = new Poke[6]{new Poke("ガブリアス"), new Poke("ミミッキュ"), new Poke("ボーマンダ"),
			new Poke("メタグロス"), new Poke("バシャーモ"), new Poke("ゲンガー")};
		...
		{
			PokeLevel = (int)NumLevel.Value;

			// 各パーティーの基礎ステータスを計算し、HPを初期化する
			foreach (Poke poke in MyParty)
			{
				poke.Level = PokeLevel;
				// とりあえずステータスを計算しておく
				poke.Status = Util.CalcBasicStatus(poke.Syuzoku, poke.Indi, poke.Effort, poke.Level, poke.Nature);
		...
		private void buttonAttack_Click(object sender, EventArgs e)
		{
			...
			try
			{
				move = new Move(comboBoxMove.Text);
			}
			catch (Exception ex)
			{
			...
			WriteResult("=============================================\r\n");
			WriteResult("\r\n");
			WriteResult("{0} のこうげき！ {1}\r\n", MyPoke.Name, move.Name);

			// タイプ相性を計算
			var typeMatch = Util.CalculateTypeMatching(MyPoke, EnemyPoke, move);
			if(typeMatch >= 2.0)
			{
			...
			{
				Util.ApplyItem(comboBoxItem.SelectedItem.ToString(), move, MyPoke, typeMatch);
			}

			// ダメージ計算 0が低いほうで1が高いほう
			int[] damage = Util.CalculateDamage(MyPoke, EnemyPoke, move, PokeLevel);

			// 分析
			var percentLow = ((double)damage[0] / EnemyPoke.HPRemain) * 100.0;
			var percentHigh = ((double)damage[1] / EnemyPoke.HPRemain) * 100.0;
			...
			// デモ用
			EnemyPoke.HPRemain = EnemyPoke.HPRemain - (damage[1] + damage[0])/2;
			if(EnemyPoke.HPRemain == 0)
			{
				WriteResult("{0} はたおれた！\r\n", EnemyPoke.Name);
			}
		...
		private void AttackPoke_Changed()
		{
			...
			using (var cn = new SQLiteConnection(cBuilder.ToString()))
			{
				...
				using (var cmd = new SQLiteCommand(cn))
				{
					...
					while (reader.Read())
					{
						comboBoxMove.Items.Add(reader["waza"].ToString());
					}
				}
			}
		}
		...
		private void panelPoke_DragDrop(object sender, DragEventArgs e)
		{
			if (e.Data.GetDataPresent(typeof(Poke)))
			{
				...
				if (Poke.IsMyParty)
				{
					MyPoke = destPic.Poke = Poke;
					MyPoke.IsAttack = true; // TODO 本当はここにあるべきではない。
					AttackPoke_Changed();
					UpdateProgressBars(true);
				}
				else
				{
					EnemyPoke = destPic.Poke = Poke;
					EnemyPoke.IsAttack = false;
					UpdateProgressBars(false);
				}
			}
		}
		...
		{
			MyPoke.HPRemain = MyPoke.StatusH;
			EnemyPoke.HPRemain = EnemyPoke.StatusH;
			UpdateProgressBars(true);
			UpdateProgressBars(false);
		}
		...
		private void UpdateProgressBars(bool IsAttackPoke)
		{
			if (IsAttackPoke)
			{
				progressBarMyPoke.Maximum = MyPoke.StatusH;
				progressBarMyPoke.Value = MyPoke.HPRemain;
			}
			else
			{
				progressBarEnemyPoke.Maximum = EnemyPoke.StatusH;
				progressBarEnemyPoke.Value = EnemyPoke.HPRemain;
			}
		}
